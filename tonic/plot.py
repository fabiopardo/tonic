...
def stats(xs, means, stds):
    ...
    assert len(xs) == 1 or np.all(xs[1:] == xs[0])
    x = xs[0]
...
def get_data(
    paths, baselines, baselines_source, x_axis, y_axis, x_max, window
):
    ...
    for path in log_paths:
        ...
        if file == 'log.csv':
            ...
            # Load the data.
            dfs[seed] = pd.read_csv(path, sep=',', engine='python')
    ...
    if baselines:
        ...
        if baselines_source == 'tensorflow':
            ...
            logger.log('Loading baselines from {}...'.format(baselines_source))
        ...
        for env, df_env in df.groupby('environment'):
            ...
            for agent, df_agent in df_env.groupby('agent'):
                ...
                if agent in data[env]:
                    logger.warning('Renaming baseline {}'.format(agent))
...
def plot(
    ...
    columns, x_max, baselines, baselines_source, name, save_formats, show,
    font_size, font_family, backend, dpi
):
    ...
    if num_agents <= 10:
        colors = plt.get_cmap('tab10')(range(num_agents))
    elif num_agents <= 20:
        colors = plt.get_cmap('tab20')(range(num_agents))
    else:
        colors = list(plt.get_cmap('rainbow')(np.linspace(0, 1, num_agents)))
    ...
    for env, ax in zip(envs, axes):
            ...
            for agent in sorted(data[env], key=str.casefold):
                ...
                if interval == 'std':
                    if std is None:
                        ...
                        ax.fill_between(
                            x, mean - std, mean + std, color=color, alpha=0.2,
                            lw=0)
                elif interval == 'bounds':
                    ax.fill_between(
                        x, min_mean, max_mean, color=color, alpha=0.2, lw=0)
        ...
        ax.get_yaxis().set_major_formatter(
            mpl.ticker.FuncFormatter(
                lambda x, p: format(x if x % 1 else int(x), ',')))
    ...
    for color in colors:
        marker = lines.Line2D(
            ...
            markersize=10, linewidth=0, markeredgewidth=0)
        handles.append(marker)

    # Find a nice number of legend columns. Labels should not overlap.
    for ncol in range(num_agents, 0, -1):
    ...
    # Show the plot on screen if needed.
    if show:
        try:
            plt.pause(0)
        except:
